...
an evaluation training session: directly launches game_controller's
training_controller (modality:=evaluation, real classifier output from the
pipeline via probability_topic) and the passive wheel in training mode,
plus an XDF recorder for the session (the recorder node is built directly
here, same as calibration_lsl.launch.py) -- no dependency on
game_controller's own training.launch.py file or on
ros2neuro_recorder_xdf's launch file.
...
Usage:
    ...
    ros2 launch launchers_bci evaluation_lsl.launch.py thresholds:="[0.75, 0.25]"
"""
        ...
        parameters=[
            {
                ...
                "probability_topic": LaunchConfiguration("probability_topic"),
            }
        ],
    )
