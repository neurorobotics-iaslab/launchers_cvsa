"""General launcher for the ros2neuro BCI pipeline, g.tec g.USBamp
acquisition backend.

Combines, for now: EEG acquisition (g.USBamp backend, hardcoded), XDF
session recording, and the two-class threshold game controller. More nodes
(game_bridge, etc.) will be added here later.
...
mis-timed data rather than fail loudly. `samplerate` does have a default
(512, the g.USBamp's own default) since, unlike framerate, the device
reports it isn't required to run -- see
ros2neuro_acquisition_gusbamp/config/gusbamp_configuration.yaml.

Usage:
    ros2 launch launchers_bci gtec/bci.launch.py framerate:=512
    ros2 launch launchers_bci gtec/bci.launch.py framerate:=512 subject:=sub-01 session:=01 threshold_1:=0.25
"""
...
def generate_launch_description() -> LaunchDescription:
    gusbamp_default_config = (
        get_package_share_directory("ros2neuro_acquisition_gusbamp") + "/config/gusbamp_configuration.yaml"
    )

    launch_args = [
        DeclareLaunchArgument(
            "acquisition_config",
            default_value=gusbamp_default_config,
            description="Acquisition parameter file (g.USBamp backend's config).",
        ),
        DeclareLaunchArgument(
            "framerate",
        ...
        DeclareLaunchArgument(
            "samplerate",
            default_value="512",
            description="g.USBamp hardware sample rate in Hz.",
        ),
    ...
    acquisition_launch = IncludeLaunchDescription(
        ...
        launch_arguments={
            ...
            # gusbamp_configuration.yaml also substitutes $(var samplerate),
            # unlike the LSL config -- acquisition.launch.xml doesn't
            # declare it, but passing it here still makes it resolvable
            # when the yaml is loaded with allow_substs="true".
            "samplerate": LaunchConfiguration("samplerate"),
        }.items(),
    )
